...
const deployOrJoin = async (providers: VerifierProviders, rli: Interface): Promise<DeployedVerifierContract | null> => {
  while (true) {
    ...
    switch (choice) {
      case '1': {
        const secret = await rli.question('Enter the initial secret number: ');
        return await api.deploy(providers, { secret: parseInt(secret) });
      }
      case '2':
...
const mainLoop = async (providers: VerifierProviders, rli: Interface): Promise<void> => {
  ...
  while (true) {
    ...
    switch (choice) {
      case '1': {
        const id = await rli.question('Enter the public ID (hash of the secret): ');
        await api.verify(verifierContract, id);
        break;
      }
      case '2':
